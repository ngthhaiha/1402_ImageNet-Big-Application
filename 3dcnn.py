...
def compute_losses(model, observed_app, motion, target_app, args=None):
    ...
    args = args or argparse.Namespace(**DEFAULTS)
    total_loss = (
...
def evaluate_frame_auc(model, loader, gt_concat, testing_frame_counts, device, save_dir=None, suffix="test", motion_w=0.5, frame_w=0.5, train_stats=None):
    ...
    if save_dir is not None:
        ...
        auc = float(save_evaluation_curves(scores_eval, gt_eval, str(curves_dir), np.asarray(testing_frame_counts) - 4))

    return {"auc": auc, "raw_auc": raw_auc, "frame_scores": frame_scores}
...
def load_gt_labels(dataset_base_dir: Path, dataset_name: str):
    ...
    gt_items = sorted(gt.items(), key=lambda item: _gt_sort_key(item[0]))
