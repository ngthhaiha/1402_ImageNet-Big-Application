...
def _select_chunk_subset(dataset, training_sample_indices, chunk_offset):
    if training_sample_indices is None:
        return dataset

    indices = np.asarray(training_sample_indices, dtype=np.int64)
    chunk_len = len(dataset)
    mask = (indices >= chunk_offset) & (indices < chunk_offset + chunk_len)
    local_indices = indices[mask] - chunk_offset
    if len(local_indices) == 0:
        return None
    return Subset(dataset, local_indices.tolist())


def train(config, training_chunked_samples_dir, testing_chunked_samples_file, training_sample_indices=None):
    ...
    cfg_src = config.get("cfg_file", "./cfgs/cfg.yaml")
    ...
    for epoch in range(epoch_last, epochs + epoch_last):
        ...
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            chunk_len = len(dataset)
            dataset = _select_chunk_subset(dataset, training_sample_indices, chunk_offset)
        ...
        if epoch % config["saveevery"] == config["saveevery"] - 1:
            ...
            cal_training_stats(config, model_save_path + "-%d" % (epoch + 1), training_chunked_samples_dir,
                               stats_save_path, training_sample_indices=training_sample_indices)
...
def cal_training_stats(config, ckpt_path, training_chunked_samples_dir, stats_save_path, training_sample_indices=None):
    device = config["device"]
    model = HFVAD(num_hist=config["model_paras"]["clip_hist"],
                  num_pred=config["model_paras"]["clip_pred"],
                  config=config,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cfg_file", type=str, default="./cfgs/cfg.yaml")
