...
STAGE1_BASE_CFG=${STAGE1_BASE_CFG:-cfgs/ml_memAE_sc_cfg.yaml}
EPOCHS=${EPOCHS:-50}
...
STAGE1_BATCHSIZE=${STAGE1_BATCHSIZE:-8}
# Keep Avenue conservative by default: with Docker/WSL-style /dev/shm=64M,
# DataLoader workers can crash with "Bus error" on the large Stage 2 batches.
# Override with NUM_WORKERS=N only when the shared-memory mount is large enough.
...
STAGE1_RUN_NAME=${STAGE1_RUN_NAME:-avenue_ML_MemAE_SC_kfold${KFOLD}_e${STAGE1_EPOCHS}_seed${SEED}}
RUN_NAME=${RUN_NAME:-avenue_ML_MemAE_SC_CVAE_kfold${KFOLD}_e${EPOCHS}_seed${SEED}}
...
ML_MEMAE_PRETRAINED=${ML_MEMAE_PRETRAINED:-./ckpt/avenue_ML_MemAE_SC/best.pth}
...
stage1_batchsize = int("$STAGE1_BATCHSIZE")
...
for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(np.arange(total_samples))):
    ...
    stage1_cfg["dataset_name"] = "avenue"
    ...
    stage1_cfg["batchsize"] = stage1_batchsize
  ...
  for fold in $FOLDS; do
    ...
    if [[ "$STAGES" == "all" || "$STAGES" == "stage1" ]]; then
      ...
      if [[ -d "$STAGE1_EXP_DIR" ]]; then
        echo "Refusing to reuse existing Stage 1 experiment dir before training: $STAGE1_EXP_DIR" >&2
    ...
    if [[ "$STAGES" == "all" || "$STAGES" == "stage2" ]]; then
      ...
      if [[ -d "$EXP_DIR" ]]; then
        echo "Refusing to reuse existing Stage 2 experiment dir before training: $EXP_DIR" >&2
...
for fold in $FOLDS; do
  ...
  if [[ "$STAGES" == "all" || "$STAGES" == "stage1" ]]; then
    ...
    if [[ "$RESUME" == "1" && -d "$STAGE1_EXP_DIR" ]]; then
      ...
      if [[ -n "$STAGE1_RESUME_INFO" ]]; then
        ...
        if [[ "$STAGE1_RESUME_EPOCH" -ge "$STAGE1_EPOCHS" ]]; then
          echo "==== Stage 1 ML-MemAE-SC Avenue fold=$fold already reached epoch $STAGE1_RESUME_EPOCH/$STAGE1_EPOCHS; skipping ===="
        else
          RUN_STAGE1_EPOCHS=$((STAGE1_EPOCHS - STAGE1_RESUME_EPOCH))
          echo "==== RESUME Stage 1 Avenue fold=$fold from $STAGE1_RESUME_CKPT epoch=$STAGE1_RESUME_EPOCH remaining_epochs=$RUN_STAGE1_EPOCHS ===="
        fi
      else
        echo "==== RESUME Stage 1 Avenue fold=$fold requested, but no model.pth-N found; starting fresh in $STAGE1_EXP_DIR ===="
      fi
    ...
    if [[ "$STAGE1_RESUME_EPOCH" -lt "$STAGE1_EPOCHS" ]]; then
      echo "==== Stage 1 ML-MemAE-SC Avenue fold=$fold/$((KFOLD - 1)) epochs=$RUN_STAGE1_EPOCHS target_total=$STAGE1_EPOCHS run=$STAGE1_RUN_NAME ===="
...
import ml_memAE_sc_train as s1
...
PY
  ...
  if [[ "$STAGES" == "all" || ( "$STAGES" == "stage2" && -f "ckpt/${STAGE1_RUN_NAME}/fold_${fold}/best.pth" ) ]]; then
  ...
  if [[ "$RESUME" == "1" && -d "$EXP_DIR" ]]; then
    ...
    if [[ -n "$RESUME_INFO" ]]; then
      ...
      if [[ "$RESUME_EPOCH" -ge "$EPOCHS" ]]; then
        echo "==== Stage 2 CVAE/HF2VAD Avenue fold=$fold already reached epoch $RESUME_EPOCH/$EPOCHS; skipping ===="
        continue
      fi
      RUN_EPOCHS=$((EPOCHS - RESUME_EPOCH))
      echo "==== RESUME Stage 2 Avenue fold=$fold from $RESUME_CKPT epoch=$RESUME_EPOCH remaining_epochs=$RUN_EPOCHS ===="
    else
      echo "==== RESUME Stage 2 Avenue fold=$fold requested, but no model.pth-N found; starting fresh in $EXP_DIR ===="
    fi
  ...
  echo "==== Stage 2 CVAE/HF2VAD Avenue fold=$fold/$((KFOLD - 1)) epochs=$RUN_EPOCHS target_total=$EPOCHS run=$RUN_NAME ===="
