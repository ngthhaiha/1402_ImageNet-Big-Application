...
DEFAULTS = {
    ...
    "frame_score_weight": 0.4,
    "relation_score_weight": 0.2,
    "graph_layers": 2,
    "graph_dropout": 0.10,
    "graph_alpha": 0.10,
    "graph_topk": 8,
...
def parse_args():
    ...
    parser.add_argument("--eval_test_during_train", action="store_true", help="Log test AUC during train; never used for checkpoint selection")
    parser.add_argument("--detach_recon_motion", action="store_true", help="Stop prediction loss from updating the motion reconstruction branch")
    ...
    parser.add_argument("--disable_graph", action="store_true", help="Run the original 3DResNet2 pipeline without graph interaction")
    parser.add_argument("--disable_frame_grouping", action="store_true", help="Use standard DataLoader batches instead of grouping objects by pred_frame")
    parser.add_argument("--graph_layers", type=int, default=DEFAULTS["graph_layers"])
    parser.add_argument("--graph_dropout", type=float, default=DEFAULTS["graph_dropout"])
    parser.add_argument("--graph_alpha", type=float, default=DEFAULTS["graph_alpha"], help="Initial residual weight for graph features")
    parser.add_argument("--graph_topk", type=int, default=DEFAULTS["graph_topk"], help="Keep top-k bbox neighbors per object; <=0 keeps dense same-frame graph")
    parser.add_argument("--graph_proximity_weight", type=float, default=DEFAULTS["graph_proximity_weight"], help="Weight for center-distance proximity edges in addition to IoU")
...
def bbox_iou_matrix(boxes: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    ...
    inter_w = (rb_x - lt_x).clamp_min(0.0)
    inter_h = (rb_y - lt_y).clamp_min(0.0)
    inter = inter_w * inter_h

    area = ((x2 - x1).clamp_min(0.0) * (y2 - y1).clamp_min(0.0)).clamp_min(eps)
    union = area[:, None] + area[None, :] - inter
    return inter / union.clamp_min(eps)


def build_bbox_adjacency(
    boxes: torch.Tensor,
    ...
    boxes = boxes.float()
    pred_frame = pred_frame.reshape(-1)
    same_frame = pred_frame[:, None].eq(pred_frame[None, :])

    iou = bbox_iou_matrix(boxes, eps=eps)
    centers = torch.stack(((boxes[:, 0] + boxes[:, 2]) * 0.5, (boxes[:, 1] + boxes[:, 3]) * 0.5), dim=1)
    wh = torch.stack(((boxes[:, 2] - boxes[:, 0]).abs(), (boxes[:, 3] - boxes[:, 1]).abs()), dim=1)
    ...
    proximity = torch.exp(-dist / scale)

    adj = (iou + proximity_weight * proximity) * same_frame.float()
    eye = torch.eye(n, device=boxes.device, dtype=adj.dtype)
    adj = torch.maximum(adj, eye)

    if topk is not None and topk > 0 and topk + 1 < n:
        _, nn_idx = torch.topk(adj, k=topk + 1, dim=1)
...
class ObjectGraphLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        return x + self.update(self.message(neigh))


class BBoxGraphInteraction(nn.Module):
    def __init__(
        self,
        channels: int,
        num_layers: int,
        ...
        self.layers = nn.ModuleList([ObjectGraphLayer(channels, dropout) for _ in range(num_layers)])
    ...
    def forward(self, latent: torch.Tensor, boxes: Optional[torch.Tensor], pred_frame: Optional[torch.Tensor]):
        if boxes is None or pred_frame is None or latent.shape[0] <= 1:
            stats = {"graph_alpha": 0.0, "graph_edge_density": 0.0}
            return latent, stats

        b, c, _, _, _ = latent.shape
        node = latent.mean(dim=(2, 3, 4))
        adj = build_bbox_adjacency(boxes, pred_frame, self.topk, self.proximity_weight)

        h = self.input_norm(node)
        ...
        edge_density = float((adj > 0).float().mean().detach().cpu().item())
        stats = {"graph_alpha": float(alpha.detach().cpu().item()), "graph_edge_density": edge_density}
        return enhanced, stats


def relation_anomaly_from_attention(att: torch.Tensor, shape: Tuple[int, int, int, int, int]) -> torch.Tensor:
    b, _, t, h, w = shape
    confidence_gap = att.max(dim=1).values - att.min(dim=1).values
    relation_anomaly = 1.0 - confidence_gap
    return relation_anomaly.view(b, t, h, w).mean(dim=(1, 2, 3))
...
class HF2VADLike3DResNet(nn.Module):
    ...
    def __init__(
        ...
        self.motion_graph = BBoxGraphInteraction(
            channels=fea_dim,
    ...
    def forward(self, observed_app, motion, bbox=None, pred_frame=None):
        ...
        graph_stats = {"graph_alpha": 0.0, "graph_edge_density": 0.0}
        if self.motion_graph is not None:
            motion_latent, graph_stats = self.motion_graph(motion_latent, bbox, pred_frame)
        ...
        aux = {
            ...
            "query": query,
            "mem_read": mem_read,
            "relation_anomaly": relation_anomaly_from_attention(att, motion_latent.shape),
            **graph_stats,
        }
...
def compute_losses(model, observed_app, motion, target_app, bbox, pred_frame_id, args):
    recon_motion, pred_frame, aux = model(observed_app, motion, bbox, pred_frame_id)
...
@torch.no_grad()
def compute_sample_errors(model, observed_app, motion, target_app, bbox, pred_frame_id):
    recon_motion, pred_frame, aux = model(observed_app, motion, bbox, pred_frame_id)
    motion_err = ((recon_motion - motion) ** 2).mean(dim=(1, 2, 3, 4))
    frame_err = ((pred_frame - target_app) ** 2).mean(dim=(1, 2, 3))
    relation_err = aux["relation_anomaly"]
    return motion_err, frame_err, relation_err
...
def collect_train_score_stats(model, loader, device, keep_scores=False):
    ...
    frame_errs = []
    relation_errs = []
    for observed_app, motion, target_app, bbox, pred_frame in tqdm(loader, desc="collect train stats", leave=False):
        ...
        pred_frame = pred_frame.to(device, non_blocking=True)
        motion_err, frame_err, relation_err = compute_sample_errors(model, observed_app, motion, target_app, bbox, pred_frame)
        motion_errs.extend(motion_err.cpu().numpy().tolist())
        frame_errs.extend(frame_err.cpu().numpy().tolist())
        relation_errs.extend(relation_err.cpu().numpy().tolist())

    motion_errs = np.asarray(motion_errs, dtype=np.float32)
    frame_errs = np.asarray(frame_errs, dtype=np.float32)
    relation_errs = np.asarray(relation_errs, dtype=np.float32)
    stats = {
        ...
        "relation_mean": float(relation_errs.mean()) if len(relation_errs) else 0.0,
        "relation_std": float(relation_errs.std() + 1e-8) if len(relation_errs) else 1.0,
    }
    if keep_scores:
        stats["motion_training_stats"] = motion_errs
        stats["frame_training_stats"] = frame_errs
        stats["relation_training_stats"] = relation_errs
    return stats
...
def save_training_stats(model, loader, device, path: Path):
    ...
    hf_stats = {
        ...
        "relation_training_stats": stats["relation_training_stats"],
...
def load_training_stats(path: Path):
    ...
    if "relation_mean" not in stats:
        relation_scores = np.asarray(stats.get("relation_training_stats", []), dtype=np.float32)
        stats["relation_mean"] = float(relation_scores.mean()) if len(relation_scores) else 0.0
        stats["relation_std"] = float(relation_scores.std() + 1e-8) if len(relation_scores) else 1.0
    return {
        ...
        "relation_mean": float(stats["relation_mean"]),
        "relation_std": float(stats["relation_std"]),
    }
...
def evaluate_frame_auc(
    ...
    relation_mean = train_stats["relation_mean"] if train_stats else 0.0
    relation_std = train_stats["relation_std"] if train_stats else 1.0

    obj_idx = 0
    for observed_app, motion, target_app, bbox, pred_frame in tqdm(loader, desc="Eval", leave=False):
        batch_size = int(observed_app.shape[0])
        frame_ids = pred_frame.cpu().numpy()
        observed_app = observed_app.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        motion = motion.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        target_app = target_app.to(device, non_blocking=True)
        bbox = bbox.to(device, non_blocking=True)
        ...
        relation_err = relation_err.cpu().numpy()
        motion_z = (motion_err - motion_mean) / max(motion_std, 1e-8)
        frame_z = (frame_err - frame_mean) / max(frame_std, 1e-8)
        relation_z = (relation_err - relation_mean) / max(relation_std, 1e-8)
        scores = motion_w * motion_z + frame_w * frame_z + relation_w * relation_z
    ...
    empty_score = (
        ...
        + relation_w * ((0.0 - relation_mean) / max(relation_std, 1e-8))
    )
...
def evaluate_loss(model, loader, device, args):
    ...
    for observed_app, motion, target_app, bbox, pred_frame in tqdm(loader, desc="val", leave=False):
        observed_app = observed_app.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        motion = motion.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        target_app = target_app.to(device, non_blocking=True)
        bbox = bbox.to(device, non_blocking=True)
        pred_frame = pred_frame.to(device, non_blocking=True)
        total_loss, loss_dict = compute_losses(model, observed_app, motion, target_app, bbox, pred_frame, args)
...
def train_one_epoch(model, loader, optimizer, scaler, device, use_amp, max_grad_norm, args):
    ...
    for observed_app, motion, target_app, bbox, pred_frame in pbar:
        observed_app = observed_app.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        motion = motion.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
        target_app = target_app.to(device, non_blocking=True)
        bbox = bbox.to(device, non_blocking=True)
...
def build_save_dir(args):
    ...
    return Path("./outputs") / f"graph_3dresnet2_{args.dataset_name}"
...
def save_hf_snapshot(save_dir: Path, model, optimizer, epoch: int, step: int, best_metric=None, metric_name="val_loss", max_to_save: int = 5) -> Path:
    save_dir.mkdir(parents=True, exist_ok=True)
...
def dataset_pred_frames(dataset) -> np.ndarray:
    if hasattr(dataset, "get_pred_frames"):
        return np.asarray(dataset.get_pred_frames(), dtype=np.int64)
    if isinstance(dataset, Subset):
        parent_frames = dataset_pred_frames(dataset.dataset)
        return parent_frames[np.asarray(dataset.indices, dtype=np.int64)]
    raise TypeError("Frame-grouped graph batches require ChunkedSamplesDataset or Subset thereof")


class FrameGroupedBatchSampler(Sampler[List[int]]):
    def __init__(self, dataset, max_objects: int, shuffle: bool):
        self.frames = dataset_pred_frames(dataset)
        self.max_objects = max(1, int(max_objects))
        self.shuffle = shuffle
        self.groups: Dict[int, List[int]] = {}
        for idx, frame_id in enumerate(self.frames.tolist()):
            self.groups.setdefault(int(frame_id), []).append(idx)
        self.frame_ids = list(self.groups.keys())

    def _batches_for_order(self, frame_ids: List[int]) -> List[List[int]]:
        batches = []
        cur = []
        cur_size = 0
        for frame_id in frame_ids:
            group = self.groups[frame_id]
            if cur and cur_size + len(group) > self.max_objects:
                batches.append(cur)
                cur = []
                cur_size = 0
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        yield from self._batches_for_order(frame_ids)

    def __len__(self):
        return len(self._batches_for_order(self.frame_ids))


def make_loader(dataset, batch_size, shuffle, num_workers, pin_memory, group_by_frame=False):
    if group_by_frame:
        sampler = FrameGroupedBatchSampler(dataset, batch_size, shuffle)
        return DataLoader(dataset, batch_sampler=sampler, num_workers=num_workers, pin_memory=pin_memory)
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=pin_memory)


def should_group_graph_batches(args) -> bool:
    return (not args.disable_graph) and (not args.disable_frame_grouping)
...
def run_standard_training(args, device, use_amp, train_dataset, test_dataset, gt_concat, testing_frame_counts, save_dir):
    ...
    train_loader = make_loader(train_subset, args.batch_size, True, args.num_workers, device.type == "cuda", group_by_frame=should_group_graph_batches(args))
    train_stats_loader = make_loader(train_subset, args.batch_size, False, args.num_workers, device.type == "cuda", group_by_frame=should_group_graph_batches(args))
    val_loader = make_loader(val_subset, args.batch_size, False, args.num_workers, device.type == "cuda", group_by_frame=should_group_graph_batches(args))
    test_loader = None
    if args.eval_test_during_train:
        ...
        test_loader = make_loader(test_dataset, args.batch_size, False, args.num_workers, device.type == "cuda", group_by_frame=should_group_graph_batches(args))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            if args.eval_test_during_train:
                ...
                audit_metrics = evaluate_frame_auc(
                    ...
                    relation_w=args.relation_score_weight,
        ...
        result["training_stats_summary"] = {
            ...
            "relation_mean": train_stats["relation_mean"],
            "relation_std": train_stats["relation_std"],
        }
...
def run_standard_test(args, device, train_dataset, test_dataset, gt_concat, testing_frame_counts, save_dir):
    ...
    if stats_path.exists():
        ...
        train_stats_loader = make_loader(stats_dataset, args.batch_size, False, args.num_workers, device.type == "cuda", group_by_frame=should_group_graph_batches(args))
