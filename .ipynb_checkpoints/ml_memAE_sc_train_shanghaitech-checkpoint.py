...
import ml_memAE_sc_eval_shanghaitech as ml_memAE_sc_eval
...
def train(config, training_chunked_samples_dir, testing_chunked_samples_path):
    ...
    cfg_src = config.get("cfg_file", "./cfgs/ml_memAE_sc_cfg_shanghaitech.yaml")
    shutil.copyfile(cfg_src, os.path.join(config["log_root"], config["exp_name"], "ml_memAE_sc_cfg.yaml"))
            ...
            for idx, train_data in tqdm(
                ...
                if step % config["logevery"] == config["logevery"] - 1:
                    ...
                    writer.add_figure(
                        "img/train_output",
                        visualize_sequences(
                            img_batch_tensor2numpy(out["recon"].detach().cpu()[:num_vis, :, :, :]),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cfg_file", type=str, default="./cfgs/ml_memAE_sc_cfg_shanghaitech.yaml")
