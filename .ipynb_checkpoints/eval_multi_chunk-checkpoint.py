...
def _gt_sort_key(k):
    name = os.path.splitext(str(k))[0]   # 01_0014.npy -> 01_0014
    a, b = name.split("_")
    return (int(a), int(b))
...
def evaluate(config, ckpt_path, testing_chunked_samples_path, training_stats_path, suffix):
    ...
    of_mean = of_std = frame_mean = frame_std = None
    if training_stats_path is not None:
        training_scores_stats = compat_load(training_stats_path)
        of_mean = np.mean(training_scores_stats["of_training_stats"])
        of_std = np.std(training_scores_stats["of_training_stats"])
        frame_mean = np.mean(training_scores_stats["frame_training_stats"])
        frame_std = np.std(training_scores_stats["frame_training_stats"])
    ...
    for chunk_idx, chunk_file in enumerate(chunk_files):
        dataset_test = Chunked_sample_dataset(chunk_file)
        ...
        for test_data in tqdm(
            ...
            frame_scores_local = np.sum(np.sum(np.sum(loss_frame_test, axis=3), axis=2), axis=1)

            if training_stats_path is not None:
                of_scores = (of_scores - of_mean) / of_std
                frame_scores_local = (frame_scores_local - frame_mean) / frame_std

            scores = config["w_r"] * of_scores + config["w_p"] * frame_scores_local
    ...
    for i in range(len(frame_scores)):
        if len(frame_bbox_scores[i]) == 0:
            if training_stats_path is None:
                ...
                frame_scores[i] = config["w_r"] * (0 - of_mean) / of_std + \
                                  config["w_p"] * (0 - frame_mean) / frame_std
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="optional precomputed training stats path"
    )
    ...
    if args.training_stats_path is None:
        ...
        training_stats_path = os.path.join("./eval", config["exp_name"], "training_stats_multi.npy")
        cal_training_stats(config, args.model_save_path, training_chunked_samples_dir, training_stats_path)
    else:
