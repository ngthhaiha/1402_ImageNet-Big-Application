...
from eval_multi_chunk import evaluate
...
def compat_load(path):
    # PyTorch 2.6+ defaults weights_only=True, which breaks this repo's checkpoint loading.
    return torch.load(path, weights_only=False)


def train(config, training_chunked_samples_dir, testing_chunked_samples_path):
    paths = dict(
        log_dir="%s/%s" % (config["log_root"], config["exp_name"]),
    ...
    model = HFVAD(
        num_hist=config["model_paras"]["clip_hist"],
    ...
    ML_MemAE_SC_state_dict = compat_load(config["ML_MemAE_SC_pretrained"])["model_state_dict"]
    ...
    if config["pretrained"]:
        match = re.search(r"-(\d+)$", str(config["pretrained"]))
        if match is not None:
            epoch_last = int(match.group(1))
    ...
    # copy the actual shanghaitech config into log dir
    cfg_src = config.get("cfg_file", "./cfgs/cfg_shanghaitech.yaml")
    shutil.copyfile(cfg_src, os.path.join(paths["log_dir"], "cfg.yaml"))
    ...
    for epoch in range(epoch_last, epochs + epoch_last):
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            for idx, train_data in tqdm(
                ...
                if step % config["logevery"] == config["logevery"] - 1:
                    ...
                    num_vis = 6
                    writer.add_figure(
                        "img/train_sample_frames",
                        visualize_sequences(
                            img_batch_tensor2numpy(sample_frames.cpu()[:num_vis, :, :, :]),
                    ...
                    writer.add_figure(
                        "img/train_frame_recon",
                        visualize_sequences(
                            img_batch_tensor2numpy(out["frame_pred"].detach().cpu()[:num_vis, :, :, :]),
                    ...
                    writer.add_figure(
                        "img/train_of_recon",
                        visualize_sequences(
                            img_batch_tensor2numpy(out["of_recon"].detach().cpu()[:num_vis, :, :, :]),
                            seq_len=sample_ofs.size(1) // 2,
                            return_fig=True
                        ),
                        global_step=step + 1
        ...
        if epoch % config["saveevery"] == config["saveevery"] - 1:
            ...
            cal_training_stats(
                config,
                model_save_path + "-%d" % (epoch + 1),
                training_chunked_samples_dir,
                stats_save_path
            )
...
def cal_training_stats(config, ckpt_path, training_chunked_samples_dir, stats_save_path):
    ...
    training_stats = dict(
        of_training_stats=of_training_stats,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg_file", type=str, default="./cfgs/cfg_shanghaitech.yaml")
    ...
    testing_chunked_samples_path = os.path.join(dataset_base_dir, dataset_name, "testing/chunked_samples")

    train(config, training_chunked_samples_dir, testing_chunked_samples_path)