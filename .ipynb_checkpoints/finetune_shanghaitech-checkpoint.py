...
from utils.model_utils import saver, only_model_saver
...
def train(config, training_chunked_samples_dir, testing_chunked_samples_path):
    ...
    if not config["pretrained"]:
        ...
        model_state_dict = compat_load(config["pretrained"])["model_state_dict"]
        model.load_state_dict(model_state_dict)

    writer = SummaryWriter(paths["log_dir"])

    cfg_src = config.get("cfg_file", "./cfgs/finetune_cfg_shanghaitech.yaml")
    shutil.copyfile(cfg_src, os.path.join(paths["log_dir"], "finetune_cfg.yaml"))
...
def cal_training_stats(config, ckpt_path, training_chunked_samples_dir, stats_save_path):
    ...
    with torch.no_grad():
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            dataloader = DataLoader(dataset=dataset, batch_size=eval_batch_size, num_workers=0, shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg_file", type=str, default="./cfgs/finetune_cfg_shanghaitech.yaml")
